...
8. **Compose-then-retune pass.** After the cross-role audit, revisit each role's prompt *inside the composition*: tighten interface language against the actual inputs the role receives, strike instructions the topology now makes redundant, add the orchestration context the role needs. Prompts tuned in isolation are not yet tuned under interference (Principle 4); staged MAS-search work found this final workflow-level pass delivers converged gains after the topology is fixed (see `references/agent-team-topologies.md` §Topology optimization ordering — including the rule that a well-prompted single agent is the baseline every added block must beat).
